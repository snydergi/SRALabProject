...
def load_data():
    ...
    Returns:
        tuple:
            ...
            - timeseries (np.ndarray): Full training timeseries
            - timeseries_test (np.ndarray): Full testing timeseries
    """
...
def plot_loss(train_loss_list, test_loss_list):
    """Plot training and test loss curves with best epoch marked.
    
    Args:
        train_loss_list (list): List of training losses per epoch
        test_loss_list (list): List of test losses per epoch
    """
    # Plotting the losses
...
def plot_predictions(model, X_test, test, lookback, timeseries, timeseries_test):
    """Plot true vs predicted therapist joint positions for both training and test data.
    
    Args:
        model (nn.Module): Trained model for making predictions
        X_test (torch.Tensor): Test feature tensor
        test (np.ndarray): Test dataset for true values
        lookback (int): Lookback window size used in dataset creation
        timeseries (np.ndarray): Full training timeseries data
        timeseries_test (np.ndarray): Full testing timeseries data
    """
    train_size = int(len(timeseries))
    test_size = int(len(timeseries_test * 0.5))
    # Plotting
...
def main():
    """Main function to perform hyperparameter search using GridSearchCV with skorch."""
