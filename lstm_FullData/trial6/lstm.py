"""File that was used to train the LSTM model described in trial6/notes.txt"""
import numpy as np
...
def create_dataset(dataset, lookback):
    ...
    for i in range(len(dataset)-lookback):
        feature = dataset[i:i+lookback, :11]  # Feature is patient data
...
class JointModel(nn.Module):
    ...
    transformation layer followed by a second linear layer to produce predictions for each timestep.
    
    Architecture:
        - LSTM layer: 4 layers, 50 hidden units, processes sequences with 11 input features, dropout 0.4
        - Linear layer 1: Maps from 50-dimensional LSTM output to 128-dimensional ouput
        - Linear layer 2: Maps from 128-dimensional input to 4-dimensional prediction
    """
    def __init__(self):
